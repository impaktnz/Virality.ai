  ...
  const renderPlatformCard = (p: Platform) => {
    ...
    return (
      <div className="space-y-10 animate-fade-in-up">
        {/* Virality Analysis Overlay */}
        <div className="flex items-center justify-between gap-6 bg-dark/40 dark:bg-dark-border/50 p-8 rounded-[40px] border border-gold/30 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <svg className="w-24 h-24 text-gold" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
          </div>
          <div className="relative z-10 flex flex-col">
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Algorithm Potential</span>
            <span className="font-display text-4xl font-black text-gold italic">
              {data.virality_score}<span className="text-xl">/100</span> ⭐
            </span>
          </div>
          <div className="relative z-10 text-right flex flex-col items-end">
             <span className="text-[9px] font-black text-gold uppercase tracking-tighter block mb-1">Viral Strategy</span>
             <p className="text-[11px] font-bold text-gray-400 max-w-[200px] leading-tight italic">{data.strategy}</p>
          </div>
        </div>
            ...
            <div className="bg-dark-card border border-gold/20 rounded-[40px] p-8 space-y-6 shadow-xl relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                <div className="flex flex-col">
                  ...
                  <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mt-1">Hashtag-Free Discovery Pack</p>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Viral Emoji Deck</label>
            <div className="flex flex-wrap gap-4">
              {variant.emoji_set.map((emoji, i) => (
                <button 
                  key={i} 
                  onClick={() => handleCopy(`emoji_${p}_${i}`, emoji)}
                  className="text-3xl p-5 bg-white dark:bg-dark-border rounded-2xl border border-gray-100 dark:border-dark-border shadow-md hover:border-gold hover:scale-110 transition-all active:scale-90"
                >
                  {emoji}
                </button>
              ))}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[9px] font-black text-gold/60 uppercase tracking-widest">Algorithm Selected</span>
          </div>
        ...
        {/* Refinement Actions */}
        <div className="grid grid-cols-3 gap-4 pt-8 border-t border-gray-100 dark:border-dark-border">
           <button onClick={() => onRefine(`Make the current ${formulas[variantIndex].label} variant for ${p} even more aggressive and viral`)} className="flex flex-col items-center justify-center py-6 bg-gray-50 dark:bg-dark-border/40 rounded-[32px] gap-3 hover:bg-gold/10 transition-all border-2 border-transparent hover:border-gold/30 group">
             <span className="text-3xl group-hover:scale-125 transition-transform duration-300">🚀</span>
             <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 group-hover:text-gold">MORE VIRAL</span>
           </button>
           <button onClick={() => onRefine(`Significantly shorten the content for ${p}'s ${formulas[variantIndex].label} variant for quick reading`)} className="flex flex-col items-center justify-center py-6 bg-gray-50 dark:bg-dark-border/40 rounded-[32px] gap-3 hover:bg-gold/10 transition-all border-2 border-transparent hover:border-gold/30 group">
             <span className="text-3xl group-hover:scale-125 transition-transform duration-300">✂️</span>
             <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 group-hover:text-gold">SHORTER</span>
           </button>
           <button onClick={() => onRefine(`Regenerate ${p} with a completely different concept for the ${formulas[variantIndex].label} variant`)} className="flex flex-col items-center justify-center py-6 bg-gray-50 dark:bg-dark-border/40 rounded-[32px] gap-3 hover:bg-gold/10 transition-all border-2 border-transparent hover:border-gold/30 group">
             <span className="text-3xl group-hover:rotate-180 transition-transform duration-500">🔄</span>
             <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 group-hover:text-gold">REGENERATE</span>
           </button>
        </div>
      </div>
    );
